...
# Install gawk for script to work properly
Separate 4
printf "Installing \e[33;01mgawk\e[00m for script to work properly\n\n"
...
if [ "$load_tmp_file" = "no" ]; then
	...
	for i in $(cat "$packages_file"); do
		...
		if [ -n "$(gawk '$0 ~ /\t/' $MEMFILE)" -a "$SKIP_CATEGORY" = 'no' ]; then
...
# Auto-accept the ttf-mscorefonts EULA
...
printf "\e[01;33mPlease reboot for all changes to take effect!\e[00m\n"
exit 0
